...
- `AppLog` / `AppDiagnostics` (`Services/Logging.swift`): Application-level diagnostic logging via `os.Logger` (subsystem `com.localmgr.mac`), mirrored into an in-memory ring buffer surfaced by `DiagnosticsView` (`Cmd+Shift+L`). Distinct from `BackendRunnerManager.logOutput`, which only captures spawned model-engine subprocess stdout/stderr.
...
- **Recursive Tree & Credential Injection**: When querying repository file trees via `https://huggingface.co/api/models/<repo>/tree/main`, always pass `?recursive=true` so weight files nested in subdirectories (`gguf/`, `model/`) are discovered. Always probe `~/.cache/huggingface/token` and `ProcessInfo.processInfo.environment["HF_TOKEN"]` to attach `Authorization: Bearer <token>` for gated repository access (see shared `HFAuth` helper in `HuggingFaceAPIClient.swift`).
- **Credential Injection Applies to Downloads Too**: The `Authorization` header requirement above is not limited to browsing/listing endpoints (`/tree/main`, `/api/models/<repo>`) — it must *also* be attached to the actual `resolve/main/<file>` download request in `HubDownloaderService`. Omitting it causes an immediate 401/403 for gated repos (e.g. `meta-llama/*`), which looks like the download "instantly failing" since a fast auth rejection is indistinguishable from a network blip if not surfaced properly (see next point).
- **Never Collapse Error State With The Progress Flag**: Download/progress UI must not gate its *error message* visibility on the same boolean that also tears down the in-progress banner (e.g. `isDownloading`). Use a separate persisted field (e.g. `lastError: String?`) that outlives the progress flag, surfaced via `.alert()` or a dismissible inline banner, so failures (auth errors, HTTP errors, file I/O errors) remain visible to the user instead of flashing and disappearing in the same render pass.
- **Unauthenticated Retry Fallback on 401/403**: A stale/expired `HF_TOKEN` or cached CLI token must not permanently block repositories that are actually public — Hugging Face rejects a *bad* credential outright (401/403) rather than silently falling back to anonymous access. Whenever a request sent with an `Authorization` header comes back 401/403, retry once without the header (see `HFAuth.requestWithFallback` in `HuggingFaceAPIClient.swift`, shared by both `HuggingFaceAPIClient` and `HubDownloaderService`) before surfacing an error.
- **Distinguish 401 (Invalid/Expired Token) From 403 (Unaccepted License)**: Don't collapse both into one generic "authentication required" string — they require different user remediation. 401 means the token itself is missing/invalid (`Set HF_TOKEN` / re-run `huggingface-cli login`); 403 means the token is valid but the repo's gated license hasn't been accepted on huggingface.co (visit the repo page and accept terms). `HFAuth.describeError(statusCode:repoID:tokenWasSent:)` centralizes this so both the downloader and the Hub Discovery inspector report identical, accurate guidance.

### Application Diagnostic Logging (`AppLog`)
- **Always Log Through `AppLog`, Never `print()`/`NSLog()`**: Every service-level failure or notable state transition (network errors, gateway bind/conflict failures, process launch/termination, auto-tuner decisions, bookmark I/O errors) must go through `AppLog.debug/info/error/fault(_:category:)` in `Services/Logging.swift`, not `print()`. This both emits through Apple's unified logging system (`os.Logger`, subsystem `com.localmgr.mac` — inspectable via Console.app or `log show --predicate 'subsystem == "com.localmgr.mac"' --last 1h` even after the app has quit) and mirrors into the in-app `DiagnosticsView` (`Cmd+Shift+L`) ring buffer for live inspection and export, so a transient UI failure (e.g. a toast that flashes and disappears) is never the *only* record of what happened.
- **No Secrets or Full Paths in Log Messages**: `AppLog` messages are treated as public strings (both in unified logging and the in-app viewer/export). Never interpolate raw HF tokens, full absolute file paths, or other user-identifying data — use booleans/counts ("token present: true"), `url.lastPathComponent`, or repo IDs (which are not secret) instead.
- **Distinct From `BackendRunnerManager.logOutput`**: `AppLog`/`DiagnosticsView` covers app-internal events. It must never be conflated with the model-runner "Live Logs" tab in `ModelInspectorView`, which is exclusively for spawned engine subprocess (`llama-server`, `mlx_lm.server`, etc.) stdout/stderr — keep those two log surfaces separate rather than merging app events into the subprocess buffer.
