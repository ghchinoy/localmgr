...
- **Gateway Loopback Restriction Mechanism:** replaced `NWParameters.requiredLocalEndpoint` (from `v0.5.2`'s `[localmgr-b9v.1]` fix, which caused build/runtime issues during host verification) with the simpler, purpose-built `NWParameters.acceptLocalOnly = true`. Verified via host smoke test: unreachable from a second device on the same network, still reachable via `127.0.0.1` on this machine.
